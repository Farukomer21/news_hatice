...
class TurizmAjansiScraper:
    """
    https://www.turizmajansi.com/ haber sitesinden 
    belirtilen tarihe ait tüm haberleri ve tam metinlerini (full body) çeken kazıyıcı.
    """
    BASE_URL = "https://www.turizmajansi.com"
    HEADERS = {
    ...
    def get_article_links(self) -> List[str]:
        ...
        """
        article_links = set()
        print("🔍 Turizm Ajansı sitesindeki haber bağlantıları taranıyor...")
        ...
        for cat in self.CATEGORIES:
            ...
            try:
                ...
                if res.status_code == 200:
                    found = re.findall(r'href=[\"\'](/haber/[^\"\'\s]+|https://www.turizmajansi.com/haber/[^\"\'\s]+)', res.text)
                    for f in found:
                        if f.startswith("/"):
                            article_links.add(f"{self.BASE_URL}{f}")
                        else:
                            article_links.add(f)
    ...
    def extract_article_details(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Yayınlanma Tarihi (time etiketinin datetime özniteliği veya metni)
    ...
    def fetch_today_news(self, target_date: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        for idx, url in enumerate(all_links, 1):
            ...
            if article and article["published_date"]:
                # Tarih eşleştirmesi (Örn: '2026-08-03' tarihin içinde var mı?)
...
if __name__ == "__main__":
    ...
    output_file = os.path.join(script_dir, "turizmajansi_today_news.json")
    
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump({
            "site": "https://www.turizmajansi.com",
            "date": today_date,
